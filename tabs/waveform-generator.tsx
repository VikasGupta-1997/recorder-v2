...
//   const [contentState, setContentState] = useContext(ContentStateContext); // Access the ContentState context
  ...
  const loadWaveform = async (blob) => {
    try {
      ...
      const audioArrayBuffer = await blobToArrayBuffer(blob) as any;
  ...
  useEffect(() => {
    ...
    video.onloadedmetadata = async () => {
      const containerRect =
        waveformContainerRef.current.getBoundingClientRect();
      const cursorX =
        containerRect.width * (time / video.duration);
      customCursorRef.current.style.left = `${cursorX}px`;
  ...
  }, [time, blob, waveformContainerRef.current]);

  return (
    <div style={{ height: "100%" }}>
      <div className={style.cursor} ref={customCursorRef}></div>
      <div
        className={style.ghostCursor}
        style={showGhost ? { opacity: 1 } : { opacity: 0 }}
        ref={ghostCursorRef}
      ></div>
      <div className={style.waveform} ref={waveformContainerRef}></div>
    </div>
  );
};
